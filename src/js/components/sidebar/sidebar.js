...
export function getSidebar() {
    ...
    const sidebarHeader = createElement('header', 'sidebar__header');

    const sidebarMenu = createElement('nav', 'sidebar__menu');
    ...
    const links = {
        "home" : getMenuLink('/', 'Лента', svg1),
        "creator" : getMenuLink('/creator', 'Создать мем', svg2),
        "profile" : getMenuLink('/profile', 'Профиль', svg3)
    }



    for (const link in links) {
        sidebarMenu.append(links[link]);
    }

    sidebarHeader.append(burger)
    ...
    sidebar.append(sidebarHeader, sidebarMenu)

    return {sidebar, setActiveMenuLink}
}